...
@interface MTiToast(private)

- (MTiToast *) settings;

@end

@implementation MTiToast
...
- (void) show:(iToastType) type{
	...
	CGSize textSize =  [text boundingRectWithSize:CGSizeMake(280, 60) options:NSStringDrawingTruncatesLastVisibleLine|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:font} context:nil].size;
	...
	switch (orientation) {
		case UIDeviceOrientationPortrait:
		{
			if (theSettings.gravity == iToastGravityTop) {
				...
				point = CGPointMake(window.frame.size.width / 2, window.frame.size.height/2 + 120);
...
+ (MTiToast *) makeText:(NSString *) _text{
	MTiToast *toast = [[MTiToast alloc] initWithText:_text];
...
- (MTiToast *) setPostion:(CGPoint) _position{
